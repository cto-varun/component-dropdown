...
exports.constructErrorMessage = void 0;
 ...
 */
const constructErrorMessage = (errorMessageData, opType) => {
  ...
  if (opType === 'bulkResume') {
    if (message !== undefined && causedBy !== undefined) {
      ...
      if (causedBy.length > 0) {
        errorDescription = causedBy.map(errorObj => !EXCLUDED_ERROR_CODES.includes(errorObj.code) ? errorObj.message : '').join(' ');
      }
    }
  }
  if (opType === 'defaultChangeSub') {
    if (results !== undefined && results.length > 0) {
      errorDescription = results.map(errorObj => errorObj.description).join(' ');
    }
  }
  if (opType === 'dueAmountFetch') {
    errorDescription = 'An error occured while fetching the due amount to be paid, please retry after some time!';
  }
...
exports.constructErrorMessage = constructErrorMessage;