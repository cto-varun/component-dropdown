...
const updateState = function (oldState, updateID, updateValue) {
  let updateType = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'value';
  const newState = {};
  Object.keys(oldState).forEach(id => {
    ...
    } else if (previousElement.type && previousElement.type.startsWith('dropdown')) {
      if (previousElement.type === 'dropdown--conditional') {
        newState[id] = {
          ...previousElement,
          options: updateID === previousElement.generatedFrom ? previousElement.optionsList[updateValue] : previousElement.optionsList[oldState[previousElement.generatedFrom].value],
          value: id === updateID ? updateValue : previousElement.value
        };
      } else {
        newState[id] = {
          ...previousElement,
          options: id === updateID && updateType === 'options' ? updateValue : previousElement.options,
          value: id === updateID && updateType === 'value' ? updateValue : previousElement.value
        };
      }
    }
  });
...
const reducer = (state, action) => {
  ...
  if (action.type === 'resetValue') {
    ...
    if (previousElement.type === 'dropdown--conditional') {
      return {
        ...
        [action.id]: {
          ...previousElement,
          options: previousElement.optionsList && previousElement.optionsList[state[previousElement.generatedFrom] && state[previousElement.generatedFrom].value || previousElement.defaultValue] || undefined,
          value: undefined
        }
      };
    }
    if (previousElement.type === 'dropdown--basic') {
      return {
        ...
        [action.id]: {
          ...previousElement,
          value: previousElement.defaultValue !== undefined ? previousElement.defaultValue : undefined
        }
      };
    }
    return {
  ...
  if (action.type === 'set') {
    return {
      ...state,
      ...updateState(state, action.id, action.value, action.updateType || 'value')
    };
  }
  return state;
};
...
const getOptions = (el, state) => {
  if (el.type === 'dropdown--conditional') {
    ...
    return options && options.length ? options.map(opt => ({
  ...
  elements.forEach(element => {
    if (element.type === 'display') {
      state[element.id] = {
        type: element.type,
        element,
        defaultValue: element.defaultValue !== undefined ? element.defaultValue : undefined,
        value: element.defaultValue !== undefined ? element.defaultValue : undefined
      };
      ...
      if (element.type === 'dropdown--conditional') {
        ...
        state[element.id] = {
          ...base,
          ...typeProperties,
          options: base.optionsList && base.optionsList[base.defaultValue || state[base.generatedFrom] && state[base.generatedFrom].value] || undefined,
          value: undefined
        };
      } else {
        state[element.id] = {
...
const reset = (dispatch, elements) => _ref2 => {
  let {
    id
  } = _ref2;
  if (id) {
...
const handleUse = (id, _ref3, result) => {
  let {
    functionName,
    params,
    process
...
const createSubscribers = (id, subscribersArray) => {
  subscribersArray.forEach(subscriber => {
    ...
    if (subscriber.fn) {
      ...
      fnResult = new Function('storage', `return ${subscriber.fn}`)(value);
    ...
    if (subscriber.caller) {
      ...
      if (window[window.sessionStorage?.tabId][caller]) {
        window[window.sessionStorage?.tabId][name] = function () {
          const result = window[window.sessionStorage?.tabId][caller](...arguments);
          handleUse(id, subscriber.use, result);
        };
      }
    }
    if (subscriber.create) {
      window[window.sessionStorage?.tabId][name] = function (_ref4) {
        let {
          result = null,
          command = '',
          preprocessor = false,
        ...
        if (preprocessor) {
          const preprocessedResult = new Function('res', `return ${preprocessor}`)(payload !== undefined ? payload.value : result);
...
const useDropdown = (id, elements, saveState, subscribers) => {
  const [state, dispatch] = (0, _react.useReducer)(reducer, {}, () => defaultState(elements));
  (0, _react.useEffect)(() => {
